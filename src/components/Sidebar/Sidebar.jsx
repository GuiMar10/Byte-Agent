...
export default function Sidebar({ isOpen, onToggle }) {
  const { conversations, activeConversationId, newChat, selectConversation, deleteConversation, togglePin } = useConversations();
  const { activeModel, settings, setSettingsOpen } = useSettings();
  const [searchQuery, setSearchQuery] = useState('');
  const [contextMenu, setContextMenu] = useState(null);

  const filteredConversations = useMemo(() => {
    const nonEmpty = conversations.filter(c => c.messages && c.messages.length > 0);
    ...
    <>
      ...
      >
        <div className="sidebar__header">
          <button className="sidebar__toggle" onClick={onToggle} aria-label="Toggle sidebar" title="Toggle sidebar (Ctrl+B)">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <rect x="3" y="3" width="18" height="18" rx="2" ry="2" />
              <line x1="9" y1="3" x2="9" y2="21" />
            </svg>
          </button>
          <button className="sidebar__new-chat" onClick={handleNewChat} aria-label="New chat" title="New Chat (Ctrl+N)">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        <div className="sidebar__list" role="list">
          ...
          {filteredConversations.length === 0 && (
            <div className="sidebar__empty">
              {searchQuery ? 'Sem resultados :(' : 'Sem chats por enquanto'}
            </div>
          )}
        </div>
        ...
        <>
          ...
          <div className="context-menu" style={{ top: contextMenu.y, left: contextMenu.x }}>
            <button
              ...
              onClick={() => { togglePin(contextMenu.conv.id); closeContextMenu(); }}
            >
              {contextMenu.conv.pinned ? 'Unpin' : 'Pin'}
            </button>
            <button
              className="context-menu__item context-menu__item--danger"
              onClick={() => { deleteConversation(contextMenu.conv.id); closeContextMenu(); }}
            >
              Delete
            </button>
          </div>
...
function ChatItem({ conv, isActive, onSelect, onContextMenu }) {
  return (
    <button
      ...
      title={conv.title}
    >
      <svg className="sidebar__item-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
